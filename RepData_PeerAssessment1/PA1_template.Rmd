...
The mean for the total number of steps per day is **`r mean`**. 

The median for the total number of steps per day is **`r median`**.   
...
```
```{r,fig.height=5, fig.width=8, fig.align='center'}
plot(as.numeric(as.character(d[,1])), d[,2], type = "l", 
...
The five-minute interval that contains the maximum number of average steps per day is **`r max`**. 
...
```{r,fig.height=5, fig.width=8, fig.align='center'}
hist(f, 
     main = "Histogram of total number of steps each day(imputing missing values)", 
     xlab = "Total number of steps each day",
     ylab = "Frequency ",
     breaks=15
     )
...
The mean for the total number of steps per day is **`r mean_im`**. The corresponding mean value computed based on the dataset with missing values removed was **`r mean`**. The median for the total number of steps per day is **`r median_im`**. The corresponding mean value computed based on the dataset with missing values removed was **`r median`**. 
...
g<-as.data.frame(g);g[,2]<-rep("weekend",dim(g)[1]);g<-cbind(rownames(g),g)
h<-tapply(weekdays$steps, INDEX = weekdays$interval, FUN = "mean")
h<-as.data.frame(h);h[,2]<-rep("weekday",dim(h)[1]);h<-cbind(rownames(h),h)
colnames(h)<-colnames(g);m<-rbind(g,h)
library(lattice)
```
Draw the graph
```{r,fig.height=5, fig.width=8, fig.align='center'}
xyplot(m[,2] ~ as.numeric(as.character(m[,1])) | as.factor(m[,3]),
       data = m,
       type = 'l',
       layout = c(1,2), 
       col = 'blue',
       main = "Average number of steps by time interval (imputing missing values)",
       xlab= "5-minute time period", 
       ylab = "Average number of steps"
       )
```
